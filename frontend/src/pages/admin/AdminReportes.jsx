...
function Lightbox({ src, onClose }) {
  ...
  const descargar = () => {
    ...
    a.href = src;
    a.download = src.split('/').pop() || 'evidencia.jpg';
    a.target = '_blank';
    a.click();
  };
  return (
    ...
    <div className="min-h-screen bg-gray-950">
      ...
      <div className="page-header">
        ...
        <button onClick={() => setShowFiltros(!showFiltros)} className="text-gray-400 hover:text-white p-2 rounded-xl hover:bg-gray-800">
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2a1 1 0 01-.293.707L13 13.414V19a1 1 0 01-.553.894l-4 2A1 1 0 017 21v-7.586L3.293 6.707A1 1 0 013 6V4z" /></svg>
        </button>
      </div>
      ...
      {showFiltros && (
        <div className="mx-4 mt-2 card space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <div><label className="label">Desde</label><input type="date" className="input text-sm" value={filtros.desde} onChange={(e) => setFiltros({ ...filtros, desde: e.target.value })} /></div>
            <div><label className="label">Hasta</label><input type="date" className="input text-sm" value={filtros.hasta} onChange={(e) => setFiltros({ ...filtros, hasta: e.target.value })} /></div>
          </div>
          <div className="flex gap-2">
            <button onClick={fetchReportes} className="flex-1 btn-primary py-2.5 text-sm">Aplicar</button>
            <button onClick={() => { setFiltros({ estado: '', desde: '', hasta: '' }); setShowFiltros(false); }} className="flex-1 btn-secondary py-2.5 text-sm">Limpiar</button>
          </div>
        </div>
      )}
      ...
      <div className="p-4 space-y-3 pb-8">
        {loading ? (
          <div className="space-y-3">{[...Array(4)].map((_, i) => <div key={i} className="card animate-pulse h-20 bg-gray-800" />)}</div>
        ) : reportes.length === 0 ? (
          <div className="text-center py-16 text-gray-500"><span className="text-5xl block mb-3">📋</span><p>No hay reportes</p></div>
        ) : (
          reportes.map((r) => (
