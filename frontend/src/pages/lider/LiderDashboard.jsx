  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Último reporte por punto (todos los reportes del punto, no solo los propios)
        const mapa = {};
    ...
    <div className="min-h-screen bg-gray-950">
      ...
      <div className="page-header justify-between">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-gradient-to-br from-indigo-400 to-indigo-600 rounded-xl flex items-center justify-center">
            <span className="text-lg">👤</span>
          </div>
          <div>
            <h1 className="font-bold text-white text-sm leading-tight">Panel Líder</h1>
      ...
      <div className="p-4 space-y-4 pb-8">
        ...
        {loading ? (
          ...
          puntos.map((punto) => {
            ...
            const estado = ultimoReporte?.estado || 'ROJO';
            return (
              <button
                key={punto._id}
                onClick={() => navigate(`/lider/punto/${punto._id}`)}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {ultimoReporte ? (
                    <p className="text-gray-600 text-xs mt-0.5">
                      Últ. reporte: {new Date(ultimoReporte.fechaVisita).toLocaleDateString('es-CO', { day: '2-digit', month: 'short' })} · {ultimoReporte.usuario?.nombre}
                    </p>
                  ) : (
